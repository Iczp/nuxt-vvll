<template>
  <svg
    viewBox="34.5 34.5 443 443"
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
  >
    <linearGradient
      id="a"
      gradientUnits="userSpaceOnUse"
      x1="34.489"
      x2="478.926"
      y1="256"
      y2="256"
    >
      <stop offset="0" stop-color="#fff" />
      <stop offset="1" stop-color="#f7f7f7" />
    </linearGradient>
    <circle cx="256" cy="256" fill="url(#a)" r="221.5" />
    <path
      d="M256 287.2c17.6 0 32.2-14.6 32.2-32.2s-14.6-32.2-32.2-32.2-32.2 14.6-32.2 32.2 14.6 32.2 32.2 32.2z"
      fill="#b84297"
    />
    <g fill="#292d33">
      <path
        d="M256 60.8c-29.3 0-58.5 6.8-84.9 19.5v42c23.4-14.6 50.7-23.4 79-24.4s55.6 4.9 81 18.5 45.9 33.2 60.5 56.6 23.4 51.7 23.4 79v118.1c20.5-29.3 33.2-63.4 36.1-99.5s-4.9-71.2-21.5-103.4-41-58.5-72.2-77.1C327.2 70.6 292.1 60.8 256 60.8z"
      />
      <path
        d="M256 415.1c-41 0-80-15.6-109.3-43.9s-46.8-66.4-48.8-107.3v-121c-28.3 40-41 88.8-35.1 137.6s30.3 93.7 67.3 124.9 84.9 47.8 134.7 45.9c49.8-2 95.6-22.4 129.8-57.6v-59.5c-13.7 24.4-34.2 44.9-58.5 59.5-24.5 13.6-51.8 21.4-80.1 21.4z"
      />
      <path
        d="M256 306.7c-11.7 0-23.4-3.9-32.2-10.7v37.1c16.6 6.8 36.1 8.8 53.7 3.9s33.2-14.6 44.9-28.3 18.5-31.2 19.5-49.8-3.9-36.1-14.6-51.7c-9.8-15.6-25.4-26.3-42-33.2-17.6-5.9-36.1-6.8-53.7-1-17.6 4.9-33.2 16.6-43.9 31.2s-16.6 32.2-16.6 50.7v108.3c9.8 7.8 20.5 14.6 32.2 19.5V253.1c0-10.7 2.9-20.5 8.8-29.3s13.7-15.6 23.4-19.5c9.8-3.9 20.5-4.9 30.3-2.9 10.7 2 19.5 6.8 27.3 14.6s12.7 16.6 14.6 27.3c2 10.7 1 20.5-2.9 30.3s-10.7 17.6-19.5 23.4-18.6 8.8-29.3 9.7z"
      />
      <path
        d="M150.6 343.8V255c0-22.4 6.8-43.9 20.5-62.5 12.7-17.6 31.2-31.2 52.7-38.1s43.9-6.8 65.4 0 40 20.5 52.7 38.1 20.5 40 20.5 62.5-6.8 43.9-20.5 62.5c-12.7 18.5-31.2 31.2-52.7 38.1-21.5 6.8-43.9 6.8-65.4 0v34.2c24.4 5.9 48.8 4.9 72.2-2 23.4-6.8 44.9-20.5 61.5-38.1 16.6-18.5 28.3-40 33.2-64.4s3.9-48.8-3.9-72.2-21.5-43.9-40-60.5-41-27.3-65.4-32.2-48.8-2.9-72.2 4.9-43.9 22.4-59.5 41V92.1c-10.7 7.8-22.4 16.6-32.2 26.3v140.5c.9 31.3 12.6 60.5 33.1 84.9z"
      />
    </g>
  </svg>
</template>

<script lang="ts">
export default {
  name: 'AbpVNext',
};
</script>
